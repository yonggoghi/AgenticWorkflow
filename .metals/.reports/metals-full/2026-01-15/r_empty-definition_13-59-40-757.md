error id: file://<WORKSPACE>/predict_send_time/scala/40_train_eval_and_save_models.scala:PrintWriter#
file://<WORKSPACE>/predict_send_time/scala/40_train_eval_and_save_models.scala
empty definition using pc, found symbol in pc: 
empty definition using semanticdb
empty definition using fallback
non-local guesses:
	 -PredictOstConfig.PrintWriter#
	 -org/apache/spark/sql/functions/PrintWriter#
	 -PrintWriter#
...
object PredictOst40TrainEvalAndSaveModels {
  def run(): Unit = {
    ...
    val logWriter = new java.io.Prin@@tWriter(new java.io.BufferedWriter(new java.io.FileWriter(logPath, /*append=*/ true)))
    ...
    def dfShowString(df: org.apache.spark.sql.DataFrame, n: Int, truncate: Int = 0): String = {
      // Spark internal helper - avoids printing to console
      df._jdf.showString(n, truncate, false)
    }
  ...
  .setRawPredictionCol("pred_raw_xgbc_click")
    ...
    val clickTrainSampled = clickTrainBase.stat.sampleBy(
    ...
    logCounts(clickTrainSampled, indexedLabelColClick, "clickTrainSampled (Sales, indexedLabelClick)")

    val pipelineModelClick = new Pipeline().setStages(Array(xgbc)).fit(clickTrainSampled)
  ...
  logLine(metrics.confusionMatrix.toString())
    ...
    // If any exception is thrown above, the writer might not close; in practice spark-shell will exit anyway,
    // but we still close it on the normal path.
    logWriter.close()
  }
}
