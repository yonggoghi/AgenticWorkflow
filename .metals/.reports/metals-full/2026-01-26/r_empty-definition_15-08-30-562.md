error id: file://<WORKSPACE>/predict_send_time/data_transformation.scala:trainDF.
...
val categoryCols = (trainD@@F.columns.filter(x => categoryColNameList.map(x.endsWith(_)).reduceOption(_ || _).getOrElse(false)).distinct.filter(x => !tokenCols.contains(x) && !noFeatureCols.contains(x) && !continuousCols.contains(x))).distinct
