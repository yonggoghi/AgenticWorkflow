error id: file://<WORKSPACE>/predict_send_time/data_transformation.scala:XGBoostClassificationModel.
...
import ml.dmlc.xgboost4j.scala.spark.{XGBoostCl@@assificationModel, XGBoostClassifier, XGBoostRegressor}
...
// Suffix 배치 저장 설정
val suffixGroupSize = 2  // 1(개별), 2, 4, 8, 16(전체)
...
println(s"Suffix Group Size: $suffixGroupSize")
println("=" * 80)
...
println(s"Total raw records loaded: $totalRecords")
...
val trainRecords = trainDFRev.count()
val testRecords = testDFRev.count()

println(s"Split completed:")
println(s"  - Training records: $trainRecords (${(trainRecords.toDouble / totalRecords * 100).formatted("%.2f")}%)")
println(s"  - Training period: ${sendYmList.head} ~ ${sendYmList.last} ($period months)")
println(s"  - Test records: $testRecords (${(testRecords.toDouble / totalRecords * 100).formatted("%.2f")}%)")
println(s"  - Test period: $predictionDTSta ~ $predictionDTEnd")
println("=" * 80)
...
val continuousCols = (trainDFRev.columns
    .filter(x => Array("_cnt", "_mb", "_avg").map(x.endsWith(_)).reduceOption(_ || _).getOrElse(false))
    .distinct
    .filter(x => !tokenCols.contains(x) && !noFeatureCols.contains(x))
).distinct
val categoryCols = (trainDFRev.columns
    .filter(x => Array("_cd").map(x.endsWith(_)).reduceOption(_ || _).getOrElse(false))
    .distinct
...
println(s"Using suffix group size: $suffixGroupSize")
...
datasetsToSave.foreach { case (datasetName, dataFrame, outputPath, basePartitions) =>
    println(s"Saving transformed $datasetName data by suffix (group size: $suffixGroupSize)...")
    println(s"  Output path: $outputPath")

    (0 to 15).map(_.toHexString).grouped(suffixGroupSize).zipWithIndex.foreach { case (suffixGroup, groupIdx) =>
        println(s"  [Group ${groupIdx + 1}/${16 / suffixGroupSize}] Processing $datasetName suffixes: ${suffixGroup.mkString(", ")}")
        dataFrame
        ...
        println(s"  [Group ${groupIdx + 1}/${16 / suffixGroupSize}] $datasetName data saved")
    }
