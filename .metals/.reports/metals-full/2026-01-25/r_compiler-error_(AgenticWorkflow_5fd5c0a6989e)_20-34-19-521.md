...
// =============================================================================
// 통합 Raw Data 저장 (Suffix별, Hour별)
// =============================================================================
// Suffix별, send_hournum_cd별로 loop를 돌면서 raw data를 저장합니다.
// 메모리 효율을 위해 시간대별로 분할 저장합니다.
// =============================================================================

println("=" * 80)
println(s"Saving Raw Data for suffix: $smnSuffix")
println("=" * 80)
println(s"Sa@@ve path: $rawDataSavePath")
println(s"Partition by: send_ym, send_hournum_cd, suffix")
println(s"Hour range: ${hourRange.mkString(", ")}")
println("=" * 80)

// send_hournum_cd별로 loop를 돌면서 저장
hourRange.foreach { hourNum =>
  println(s"  Processing hour: $hourNum")
  
  val rawDFRevHour = rawDFRev
    .filter(F.col("send_hournum_cd") === hourNum)
  
  // 데이터가 있는지 확인 (take(1)로 빠르게 체크)
  if (rawDFRevHour.take(1).nonEmpty) {
    rawDFRevHour
      .repartition(10)  // 시간대별로 작은 파티션 수
      .write
    ...
    println(s"    Hour $hourNum saved")
  } else {
    println(s"    Hour $hourNum - no data, skipped")
  }
}
