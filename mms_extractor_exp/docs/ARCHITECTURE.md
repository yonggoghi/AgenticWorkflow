...
### 성능 최적화: Bigram Pre-filtering (2026-02-09)

Step 2 (EntityExtractionStep)의 Fuzzy Matching 병목을 해결하기 위해 bigram 기반 사전 필터링을 도입:

| 항목 | 최적화 전 | 최적화 후 | 개선율 |
|------|----------|----------|-------|
| **Step 2 소요 시간** | 16.2초 | 1.5초 | **11x** |
| **총 비교 횟수** | 904K | 48K | 94.7% 감소 |
| **전체 파이프라인** | ~30초 | ~15초 | 50% 감소 |

**핵심 구현** (`utils/similarity_utils.py`):
- `_build_bigram_index()`: 45K 엔티티에 대한 inverted index 구축 (~0.2초)
- `_filter_entities_by_bigram()`: 텍스트와 bigram이 겹치는 엔티티만 필터링
- 100K 미만 비교 시 단일 프로세스 사용 (joblib IPC 오버헤드 회피)
