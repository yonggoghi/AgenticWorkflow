...
```
→ 즉각적 혜택(5000원)과 서비스 가치(AI 기능)를 모두 포함. 대리점은 DAG에 포함하지 않음.
...
### 추출 제외 대상
- 광고 대상자, 일정/기간 정보, 부가 설명, 법적 고지사항

## 기대 행동 (10개 표준 동사)
`구매, 가입, 사용, 방문, 참여, 등록, 다운로드, 확인, 수령, 적립`

## 관계 동사 우선순위
1. **조건부**: `가입하면`, `구매하면`, `사용하면`, `가입후`, `구매후`
2. **결과**: `받다`, `수령하다`, `적립하다`
3. **경로**: `통해`, `이용하여` (필요 시만)

## 분석 프로세스
### Step 1: 메시지 이해 및 가치 제안 식별
...
- **사용자의 첫 번째 행동은?**
  - 온라인 링크 → 접속
  - 앱 관련 → 다운로드
  - **대리점/매장명은 제외** (main prompt에서 별도 추출)

### Step 3: DAG 구성
- **독립 경로 구성**: 각 가치 제안별로 별도 경로 생성
  - 혜택 획득 경로: 가입 → 사용 → 보상
  - 서비스 체험 경로: 가입 → 경험 → 기능 활용
- **세부 기능 표현**: 주요 기능은 분기 구조로
  - 통합 가능한 기능은 하나로
  - 독립적 기능은 별도로
- Root Node부터 시작하는 전체 흐름 구성
- 중복 제거 및 통합
