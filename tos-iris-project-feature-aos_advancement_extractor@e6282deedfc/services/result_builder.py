    ...
    def _map_programs_to_result(self, json_objects: Dict, pgm_info: Dict) -> List[Dict]:
        ...
        try:
            if (self.num_cand_pgms > 0 and
                ...
                pgm_names = json_objects['pgm']
                matched = pgm_info['pgm_pdf_tmp'][
                    pgm_info['pgm_pdf_tmp']['pgm_nm'].apply(
                        lambda x: re.sub(r'\[.*?\]', '', x) in pgm_names
                    )
                ]
                pgm_json = matched.sort_values('sim', ascending=False)[['pgm_nm', 'pgm_id']].to_dict('records')
